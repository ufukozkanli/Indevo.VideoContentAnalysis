...
def draw_obj(image, bbox):   
    ...
    blank_img=np.zeros((image_h,image_w,image_z), image.dtype)

    blank_img[x1:x2,y1:y2,:]=image[x1:x2,y1:y2,:]
    cv2.imwrite('/code2/js/obj1/{}.jpg'.format(ix_frame),blank_img)
    return 
    ...
    if show_label:        
        label = "{}".format(NUM_CLASS[class_ind]) + obj_id
...
def draw_obj_all(frame_ind,image, bboxes, CLASSES=YOLO_COCO_CLASSES, show_label=True, show_confidence = True, Text_colors=(255,255,0), rectangle_colors='', tracking=False):   
    if len(bboxes)==0:
        return
    image_h, image_w, image_z = image.shape  
    ...
    cv2.imwrite('/code2/js/obj1/{}.jpg'.format(frame_ind),blank_img)
